...
describe("Anchor", () => {
  describe("parse", () => {
    it("reads a uuid as the line it names", () => {
      // when
      const anchor = Anchor.parse("019fe578-e38b-7000-971e-04858335d7ff");
      // then
      expect(anchor).toEqual(
        expect.objectContaining({
          type: "id",
          value: "019fe578-e38b-7000-971e-04858335d7ff",
        }),
      );
    });
    ...
    it("reads anything else that is a time as the moment it names", () => {
      // when
      const anchor = Anchor.parse("2026-03-01T09:00:00Z");
      // then
      expect(anchor?.type).toEqual("timestamp");
      expect(anchor!.serialize()).toEqual("2026-03-01T09:00:00Z");
    });
    ...
    it.each([undefined, "", "not-a-time", "2026-13-45T99:00:00Z", "019fe578-e38b-7000-971e"])("anchors nothing for %p", (raw) => {
      ...
      expect(Anchor.parse(raw)).toEqual(undefined);
    });
  });
  ...
  describe("format", () => {
    it.each([
      ...
      // then
      expect(anchor!.serialize()).toEqual(raw);
    });
    ...
    it("normalises a time to the form the api and the url both carry", () => {
      ...
      const anchor = Anchor.parse("2026-03-01T09:00:00.000Z");
      // then
      expect(anchor!.serialize()).toEqual("2026-03-01T09:00:00Z");
    });
  });
