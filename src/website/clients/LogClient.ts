...
/**
 * Everything under `/containers/:id/logs`, which is the half of the container api that answers about
 * a log rather than about a container. It is split from {@link ContainerClient} along the same seam
 * the server is: `LogService` serves these two routes and `ContainerService` serves the other.
 */
export class LogClient {
  public constructor(private readonly yesttp: Yesttp) {}

  public async page(containerId: string, options: LogClient.PageOptions = {}): Promise<LogClient.Page> {
    const { json } = await this.yesttp.get<{
      ...
      searchParams: Internal.pageParams(options),
    });
    return {
  ...
  public async find(containerId: string, options: LogClient.FindOptions): Promise<string | null> {
...
namespace Internal {
   ...
   */
  function filterParams(filter?: LogClient.Filter): FilterParams {
    return {
  ...
  export function pageParams(options: LogClient.PageOptions): ListParams {
    return {
      limit: options.limit,
  ...
  export function findParams(options: LogClient.FindOptions): FindParams {
    return {
