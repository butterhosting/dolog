...
describe("useLogFilter", () => {
  ...
  describe("toRequest", () => {
    it("drops an empty pattern rather than sending one that matches everything", () => {
      // when
      const request = useLogFilter.toRequest(applied());
      // then
    ...
    it("sends no variant without a pattern for it to read", () => {
      // when
      const request = useLogFilter.toRequest(applied({ variant: LogPattern.Variant.regex }));
    ...
    it("sends the variant once there is a pattern", () => {
      // when
      const request = useLogFilter.toRequest(applied({ pattern: "boom", variant: LogPattern.Variant.regex }));
      // then
    ...
    it("resolves a relative span into instants", () => {
      // when
      const request = useLogFilter.toRequest(applied({ range: { kind: "preset", id: "last1h" } }));
    ...
    it("resolves against the clock now, so a relative span keeps sliding", () => {
      ...
      const before = useLogFilter.toRequest(value);
      // when (the same applied filter, asked again a moment later)
      const after = useLogFilter.toRequest(value);
      // then
    ...
    it("closes both ends for the one preset that has a past", () => {
      // when
      const request = useLogFilter.toRequest(applied({ range: { kind: "preset", id: "yesterday" } }));
      // then
      expect(request.filterSince).toBeInstanceOf(Temporal.Instant);
    ...
    it("passes a custom span through as chosen", () => {
      ...
      // when
      const request = useLogFilter.toRequest(applied({ range: { kind: "custom", since, until } }));
      // then
